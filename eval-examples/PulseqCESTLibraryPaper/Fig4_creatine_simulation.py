...
parser.add_argument('bmsim_filename', type=str, nargs='?', default='/Users/alexliebeskind/Desktop/pulseq-cest-library/sim-library/phantoms/creatine/creatine_3T_pH6.4_T22C_bmsim.yaml',
...
if data_flag == 'simulation':
    ...
    config_path = Path.cwd().parent.parent / "Desktop" / "pulseq-cest-library" / "sim-library" / "phantoms" / "creatine" / "creatine_3T_pH6.4_T22C_bmsim.yaml"
...
if ROI == 'n':
    ...
    if interpolation == "linear":
        ...
        Z_spectrum_corr = ppval(pp, w + dB0-0.028)

        # Correct MTR asymmetry spectrum using the same dB0 shift
        pp_mtr = csaps(w, V_MTRasym_spectrum, smooth=0.99)
        V_MTRasym_spectrum_corr = ppval(pp_mtr, w + dB0 -0.028)
    
    # Plot the average spectra
    plt.figure(figsize=(10, 4))
    plt.subplot(1, 2, 1)
    plt.plot(w, Z_spectrum_corr, "r.-")
    plt.xlim([-3.5, 3.5])
    plt.gca().invert_xaxis()
    plt.xlabel(r'$\Delta\omega$ [ppm]')
    plt.ylabel(r'Z($\Delta\omega$)')
    plt.title("Z-spectrum")
    plt.subplot(1, 2, 2)
    plt.plot(w, V_MTRasym_spectrum_corr, "r.-")
    plt.xlim([0, 3.5])
    plt.gca().invert_xaxis()
    plt.xlabel(r'$\Delta\omega$ [ppm]')
    plt.ylabel(r'$MTR_{asym}(\Delta\omega)$')
